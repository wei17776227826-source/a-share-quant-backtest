...
const docs = [
  ...
  },
  {
    ...
    items: [
      ...
      { id: 'example-oms', title: '尾盘动量隔夜策略 (OMS)' },
    ],
  },
  {
    section: '部署与运维',
...
短线的"隔夜持股法"：当日 14:30 尾盘选股买入，次日冲高卖出。通过 8 步筛选当日动量最强的一批中小市值活跃股，利用隔夜情绪惯性在 T+1 日获利了结。
...
| 3 | 换手率区间 | 5% ~ 10% |
| 4 | 流通市值 | 50 ~ 200 亿 |
| 5 | 持续放量（台阶式） | 近5日均量 > 近20日均量 × 1.2 |
| 6 | 均线多头排列 | MA5 > MA10 > MA20 > MA60 |
| 7 | 分时强势（回测跳过） | 全天 ≥80% 时间价 > 均价 |
| 8 | 回踩均价线不破（回测跳过） | 14:00-14:55 回踩不破均价 |

## 回测实现

回测使用 Filter 1-6（Filter 7/8 依赖当日分时数据，在历史回测中自动跳过）：

- **Filter 1**：当天涨跌幅在 3%-5% 之间
- **Filter 2**：量比 > 1（当日成交量 / 前5日均量）
- **Filter 5**：近5日均量 > 近20日均量 × 1.2，且近5日中 ≥60% 日成交量递增
- **Filter 6**：MA5 > MA10 > MA20 > MA60，且收盘价在 MA5 之上
...
| 涨跌幅上限(%) | 5.0 | 最高涨幅限制 |
| 量比下限 | 1.0 | 最低量比要求 |
| 持续放量倍率 | 1.2 | 近5日均量 / 近20日均量的最低比值 |

## 注意

- A 股目前是 T+1 交易制度，策略符合规则
- 适合波动性较高的中小市值股票
- 回测跳过了 Filter 7/8（分时数据），实际盘中表现可能优于回测`,
  },
  'deploy-server': {
    title: '服务器部署',
    body: `# 服务器部署
